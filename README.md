...
1. In Terminal:
    ```shell
    git clone https://github.com/lunovpn/lunovpn-linux-kernel-module.git
    cd lunovpn-linux-kernel-module/src
    ```
...
3. Now perform build and installation:
    ...
    sudo dkms add -m lunovpn -v 1.0.0
    sudo dkms build -m lunovpn -v 1.0.0
    sudo dkms install -m lunovpn -v 1.0.0
    ```
...
```shell
ln -s /path/to/your/kernel/sources /usr/src/lunovpn-1.0.0/kernel
```
