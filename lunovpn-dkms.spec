...
Source0:        https://github.com/lunovpn/lunovpn-linux-kernel-module/archive/refs/tags/v%{version}.tar.gz
...
Provides:       kmod(lunovpn.ko) = %{epoch}:%{version}-%{release}
Requires:       dkms
...
%prep
%autosetup -p1 -n lunovpn-linux-kernel-module-%{version}

# Fix the Makefile for CentOS7 since it ships coreutils from 2013.
sed -i 's/install .* -D -t\(.\+\) /mkdir -p \1 \&\& \0/' %{_builddir}/lunovpn-linux-kernel-module-%{version}/src/Makefile

# Set version in dkms.conf and Makefile
sed -i "s/^PACKAGE_VERSION=.*/PACKAGE_VERSION=\"%{version}\"/" %{_builddir}/lunovpn-linux-kernel-module-%{version}/src/dkms.conf
sed -i "s/^WIREGUARD_VERSION = .*/WIREGUARD_VERSION = %{version}/" %{_builddir}/lunovpn-linux-kernel-module-%{version}/src/Makefile

%build

%install
mkdir -p %{buildroot}%{_usrsrc}/lunovpn-%{version}/
make DESTDIR=%{buildroot} DKMSDIR=%{_usrsrc}/lunovpn-%{version}/ \
    -C %{_builddir}/lunovpn-linux-kernel-module-%{version}/src dkms-install

%post
dkms add -m lunovpn -v %{version} -q --rpm_safe_upgrade || :
dkms build -m lunovpn -v %{version} -q || :
dkms install -m lunovpn -v %{version} -q --force || :
echo "lunovpn-dkms-%{version}-%{release}" > /var/lib/dkms/lunovpn/%{version}/version

%preun
# Check if we are running an upgrade
if [ $1 -ne 0 ]; then
  WG_VERSION=$(dkms status lunovpn|grep installed|sort -r -V|awk '{print $2}'|cut -f1 -d,)
...
WG_VERSION_FILE=$(cat /var/lib/dkms/lunovpn/%{version}/version)
WG_RPM_VERSION=lunovpn-dkms-%{version}-%{release}
if [ "$WG_RPM_VERSION" = "$WG_VERSION_FILE" ]; then

    dkms remove -m lunovpn -v %{version} -q --all --rpm_safe_upgrade || :
...
%files
%{_usrsrc}/lunovpn-%{version}

%changelog
* Thu Oct 23 2024 Yuri Egorov <Amnezia> - 1.0.20241023-1
- Update to 1.0.20241023

* Thu Oct 22 2024 Yuri Egorov <Amnezia> - 1.0.20241022-1
- Update to 1.0.20241022

* Thu Feb 1 2024 Yuri Egorov <LunoVPN> - 1.0.20240201-1
...
- Remove patch: lunovpn-linux-compat-RHEL-0.0.20200215.patch

* Sat Feb 15 2020 Joe Doss <joe@solidadmin.com> - 0.0.20200215-2
- Apply patch: https://git.zx2c4.com/lunovpn-linux-compat/patch/?id=27ce49e385a87cb784368a0995f8284fd6887d8c
...
- Split lunovpn-tools back out into it's own spec
- Switch to https://git.zx2c4.com/lunovpn-linux-compat repo
- Move back to lunovpn-dkms spec
...
- Add /var/lib/dkms/lunovpn/%{version}/version
- Merge lunovpn-tools.spec and lunovpn-dkms.spec
