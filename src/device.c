...
static void wg_destruct(struct net_device *dev)
{
	...
	mutex_lock(&wg->junk_buffer_lock);
	kfree(wg->junk_buffer);
	wg->junk_buffer = NULL;
	mutex_unlock(&wg->junk_buffer_lock);
		      ...
		      struct netlink_ext_ack *extack)
{
	...
	mutex_init(&wg->junk_buffer_lock);
...
int wg_device_handle_post_config(struct wg_device *wg)
{
	...
	/* Pre-allocate junk buffer to avoid kzalloc/kfree on every handshake */
	mutex_lock(&wg->junk_buffer_lock);
	kfree(wg->junk_buffer);
	wg->junk_buffer = NULL;
	if (wg->jc && wg->jmax) {
		wg->junk_buffer = kzalloc(wg->jmax, GFP_KERNEL);
	}
	mutex_unlock(&wg->junk_buffer_lock);
