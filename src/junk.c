...
static void awg_fast_random_bytes(void *buf, int len) {
    u8 *p = buf;
    while (len >= 4) {
        *(u32 *)p = awg_fast_rand();
        p += 4;
...
static void random_char_modifier(char* buf, int len, struct wg_peer *peer) {
    ...
    for (i = 0; i < len; i++) {
        b = buf[i] % LETTER_LEN;
        buf[i] = (b < ALPHABET_LEN) ? 'a' + b : 'A' + b - ALPHABET_LEN;
    }
}
...
void jp_spec_applymods(struct jp_spec* spec, char* dest_buf, struct wg_peer* peer) {
    int i;
    struct jp_modifier* mod;
    ...
    for (i = 0; i < spec->mods_size; i++) {
        mod = &spec->mods[i];
        if(mod->func)
            mod->func(dest_buf + mod->offset, mod->buf_len, peer);
    }
}
